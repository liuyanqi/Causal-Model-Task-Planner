...
class CausalNode(object):
    ...
    def __init__(self, name, relation, relation_word):
        self._name    = name
        self._relation = relation
        self._relation_word = relation_word
    ...
    def relation_word(self):
        return self._relation_word

    def __str__(self):
        operator_str  = '{0}\n'.format(self._name)
        operator_str += '>> relation: {0}\n'.format(', '.join(map(str, self._relation)))
        operator_str += '>> relation_name: ' + str(self._relation_word) + "\n"
        return operator_str